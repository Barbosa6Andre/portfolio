...
function sobeToupeira() {
    ...
    objBuraco.src = 'imgsMole/hole-mole.png';
...
function tiraToupeira(buraco) {
    var objBuraco = document.getElementById('buraco' + buraco);
    objBuraco.src = 'imgsMole/hole.png';
    perdidos++;
    mostraPontuacao();
}
 ...
 * 
 * @param {string} display id do elemento do display
 * @param {int} valor valor a ser exibido com até 3 dígitos
 */
function mostraPontuacaoDe(display, valor) {
    // pega as imagens
    let objCentena = document.getElementById(display).children[0];
    let objDezena = document.getElementById(display).children[1];
    let objUnidade = document.getElementById(display).children[2];
    ...
    objCentena.src = 'imgsMole/caractere_' + centena + '.gif';
    objCentena.alt = centena;
    objDezena.src = 'imgsMole/caractere_' + dezena + '.gif';
    objDezena.alt = dezena;
    objUnidade.src = 'imgsMole/caractere_' + unidade + '.gif';
...
function marteloBaixo() {
    document.getElementById('idGramado').style.cursor = 'url(imgsMole/hammerDown.png), default';
}
...
function marteloCima() {
    document.getElementById('idGramado').style.cursor = 'url(imgsMole/hammer.png), default';
}
...
function martelada(evento) {
    if (evento.target.src.includes('hole-mole')) {
        // acertou
        acertos++;
        evento.target.src = 'imgsMole/hole.png';
        clearTimeout(timer);
    } else {
        // errou
        errados++;
